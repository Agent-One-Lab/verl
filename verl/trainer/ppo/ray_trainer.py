...
from ....verl.workers.utils.padding import (
    ...
    no_padding_2_padding_with_attention_mask,
)
...
def apply_kl_penalty(data: DataProto, kl_ctrl: core_algos.AdaptiveKLController, kl_penalty="kl"):
    ...
    # Align to policy-token space: log_prob at position t predicts token t+1.
    ...
    kld = core_algos.kl_penalty(
        old_log_probs, ref_log_prob, kl_penalty=kl_penalty
...
def _align_batch_to_policy_space(batch: DataProto) -> DataProto:
    """Align sequence tensors to policy-token space (L-1) once in trainer."""
    action_mask = batch.batch["action_mask"]
    policy_mask = action_mask[:, 1:]
    batch.batch["action_mask"] = policy_mask
    policy_len = policy_mask.size(1)

    for key in ("old_log_probs", "ref_log_prob", "token_level_scores", "token_level_rewards", "values", "sum_pi_squared"):
        if key not in batch.batch:
            continue
        tensor = batch.batch[key]
        if tensor.ndim < 2:
            continue
        if tensor.size(1) == policy_len:
            continue
        if tensor.size(1) == policy_len + 1:
            batch.batch[key] = tensor[:, :-1]
            continue
        raise ValueError(
            f"Cannot align `{key}` with shape {tensor.shape} to policy length {policy_len}."
        )
    return batch
...
class RayPPOTrainer:
    ...
    def init_workers(self):
        ...
        # We calculate reward in agentfly's agent loop
    ...
    def _compute_values(self, batch: DataProto) -> DataProto:
        if self.use_legacy_worker_impl == "disable":
            ...
            values = no_padding_2_padding_with_attention_mask(values, batch_td, target_mask_key="action_mask")
    ...
    def _compute_ref_log_prob(self, batch: DataProto) -> DataProto:
        if self.use_legacy_worker_impl == "disable":
            ...
            log_probs = no_padding_2_padding_with_attention_mask(log_probs, batch_td, target_mask_key="action_mask")
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # Shift action_mask: move token-indexed to state-indexed
                    action_mask_aligned = torch.zeros_like(batch.batch["action_mask"])
                    action_mask_aligned[:, :-1] = batch.batch["action_mask"][:, 1:]
                    # Now action_mask_aligned[t] = "was the action at state s_t a generated token?"
                    # Last position is 0 (no action after the final state)

                    # Same for rewards
                    reward_scores_aligned = torch.zeros_like(batch.batch["rm_scores"])
                    reward_scores_aligned[:, :-1] = batch.batch["rm_scores"][:, 1:]

                    batch.batch['action_mask'] = action_mask_aligned
                    batch.batch['rm_scores'] = reward_scores_aligned
                    ...
                    if bypass_recomputing_logprobs:  # Use `rollout_log_probs`
                        ...
                        with marked_timer("old_log_prob", timing_raw, color="blue"):
                            ...
                            if entropys.size(1) == action_mask.size(1) + 1:
                                entropys = entropys[:, :-1]
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        # Centralized policy-space alignment used by advantage and actor/critic updates.
                        # batch = _align_batch_to_policy_space(batch)
