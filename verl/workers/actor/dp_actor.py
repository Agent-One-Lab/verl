    ...
    def _forward_micro_batch(
        ...
        with torch.autocast(device_type=self.device_name, dtype=self.param_dtype):
            ...
            if self.use_remove_padding:
                ...
                if calculate_sum_pi_squared:
                    # (bsz, response_length)
                    sum_pi_squared = full_sum_pi_squared.squeeze(-1)[:, : -1]
                ...
                if self.use_fused_kernels:
                    ...
                    if calculate_entropy:
                        if not self.config.entropy_checkpointing:
                            ...
                            entropy = verl_F.masked_entropy_from_logits(logits, micro_batch["action_mask"])
                        else:
                            entropy = torch.utils.checkpoint.checkpoint(
                                verl_F.masked_entropy_from_logits, logits, micro_batch["action_mask"]
                            )
                    # Compute sum_pi_squared if requested (for optimal_token_baseline)
                    if calculate_sum_pi_squared:
                        sum_pi_squared = (
