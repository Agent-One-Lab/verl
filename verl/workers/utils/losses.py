...
from ....verl.workers.utils.padding import no_padding_2_padding, no_padding_2_padding_with_attention_mask
...
def ppo_loss(config: ActorConfig, model_output, data: TensorDict, dp_group=None):
    ...
    log_prob = no_padding_2_padding_with_attention_mask(model_output["log_probs"], data)
    entropy = model_output.get("entropy", None)
    if entropy is not None:
        entropy = no_padding_2_padding_with_attention_mask(entropy, data)
    ...
    response_mask = data["action_mask"].to(bool)
    # compute policy loss
    old_log_prob = data["old_log_probs"]
    # if log_prob.shape[1] == old_log_prob.shape[1] + 1:
...
def value_loss(config: CriticConfig, model_output, data: TensorDict, dp_group=None):
    ...
    Returns:
        value loss
    """
    vpreds = no_padding_2_padding_with_attention_mask(model_output["values"], data)  # (bsz, response_length)
    response_mask = data["action_mask"].to(bool)
