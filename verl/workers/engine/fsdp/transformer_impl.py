    ...
    def prepare_model_outputs(self, output, output_args, micro_batch: TensorDict, logits_processor_func):
        ...
        if use_remove_padding:
            ...
            if use_fused_kernels:
                ...
                # Some models (e.g. Qwen3.5 hybrid linear/full attention) can return logits on the full
                # gathered sequence while inputs/temperature were Ulysses-sliced per rank. Align lengths.
                if self.use_ulysses_sp:
                    n_log, n_temp = logits_rmpad.shape[0], temperature_rmpad.shape[0]
                    sp = self.ulysses_sequence_parallel_size
                    if n_log != n_temp and n_log == n_temp * sp and n_log % sp == 0:
                        rank = get_ulysses_sequence_parallel_rank()
                        chunk = n_log // sp
                        logits_rmpad = logits_rmpad[rank * chunk : (rank + 1) * chunk].contiguous()
