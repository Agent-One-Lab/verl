...
def _verl_log_level(*, default: int = logging.WARNING) -> int:
    """Resolve VERL_LOGGING_LEVEL to a logging level constant."""
    raw = os.getenv("VERL_LOGGING_LEVEL")
    if raw is None:
        return default
    key = raw.strip().upper()
    if key == "WARN":
        key = "WARNING"
    return getattr(logging, key, default)


# Module loggers alone cannot emit DEBUG if the root logger (Ray/Hydra/etc.) stays at WARNING.
# When the user sets VERL_LOGGING_LEVEL, align the root logger so propagation works.
_level = _verl_log_level(default=logging.WARNING)
if os.environ.get("VERL_LOGGING_LEVEL") is not None:
